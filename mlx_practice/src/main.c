...
void	rendering(t_vars *vars, t_data *img, t_scene *scene)
{
	int			i;
	int			j;
	double		u;
	...
	while (--j >= 0)
	{
		...
		while (++i < scene->canvas.width)
		{
			...
			my_mlx_pixel_put(img, i, scene->canvas.height - j - 1, create_rgb(&pixel_color));
		}
	}
...
//	rotate(scene->world, Y, PLUS);
	...
	vars.mlx = mlx_init();
	vars.win = mlx_new_window(vars.mlx, scene->canvas.width, scene->canvas.height, "miniRT");
	img.img = mlx_new_image(vars.mlx, scene->canvas.width, scene->canvas.height);
	img.addr = mlx_get_data_addr(img.img, &img.bits_per_pixel, &img.line_length, &img.endian);

	/*	rendering	*/
	rendering(&vars, &img, scene);
